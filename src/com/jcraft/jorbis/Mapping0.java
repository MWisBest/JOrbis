...
class Mapping0 extends FuncMapping
{
	...
	@Override
	void free_info( InfoMapping0 imap )
	{
	}
	
	@Override
	void free_look( InfoMapping0 imap )
	{
	}
	
	@Override
	LookMapping0 look( DspState vd, InfoMode vm, InfoMapping0 m )
	{
	...
	@Override
	void pack( Info vi, InfoMapping0 imap, Buffer opb )
	{
		/*
		...
		if( imap.coupling_steps > 0 )
		{
			opb.write( 1, 1 );
			opb.write( imap.coupling_steps - 1, 8 );
			for( int i = 0; i < imap.coupling_steps; i++ )
			{
				opb.write( imap.coupling_mag[i], Util.ilog2( vi.channels ) );
				opb.write( imap.coupling_ang[i], Util.ilog2( vi.channels ) );
			}
		}
		else
		{
		...
		if( imap.submaps > 1 )
		{
			for( int i = 0; i < vi.channels; i++ )
				opb.write( imap.chmuxlist[i], 4 );
		}
		for( int i = 0; i < imap.submaps; i++ )
		{
			opb.write( imap.timesubmap[i], 8 );
			opb.write( imap.floorsubmap[i], 8 );
			opb.write( imap.residuesubmap[i], 8 );
		}
	}
	
	// also responsible for range checking
	@Override
	InfoMapping0 unpack( Info vi, Buffer opb )
	{
